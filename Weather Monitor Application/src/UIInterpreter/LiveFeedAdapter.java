...
/**
 * UIInterpreter.LiveFeedAdapter.java
 * An adapter class which it translates the data from WeatherObserver.LocationObserver.java and pass it to the weatherFrame UI
...
public class LiveFeedAdapter implements MonitorAdapter {
    ...
    /**
     * An init function that requires all of the relevant information to construct a UIInterpreter.LiveFeedAdapter
    ...
    /**
     * Translate the data from location WeatherObserver.Observer to a desire form and pass it to weatherFrame
    ...
    public void displayTemperature(String temperature,String temperatureTimeStamp) {
        try {
            Float.parseFloat(temperature);//Checking if it's a double
    ...
    public void displayRainFall(String rainFall, String rainFallTimeStamp) {
        try {
            Float.parseFloat(rainFall); //Checking if it's a double
        } catch (NumberFormatException ex) {
            rainFall = "-";
        }
